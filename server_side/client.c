...
void login(int client_socket)
{
    ...
    int bytes_received = recv(client_socket, response_buffer, MAX_BUFFER_SIZE, 0);
    if (bytes_received == -1)
    {
    ...
    Chess__Message *response = chess__message__unpack(NULL, bytes_received, response_buffer);
    if (response == NULL)
    {
    ...
    {
    case CHESS__MESSAGE__MESSAGE_TYPE__LOGINRESPONSE:
        printf("Login response\n");
        Chess__LoginResponse *login_response = response->login_response;
