...
export function encodeLoginMessage(message) {
  let bb = popByteBuffer();
  _encodeLoginMessage(message, bb);
  return toUint8Array(bb);
}

function _encodeLoginMessage(message, bb) {
...
export function decodeLoginMessage(binary) {
  return _decodeLoginMessage(wrapByteBuffer(binary));
}

function _decodeLoginMessage(bb) {
  let message = {};
...
function _encodeMessage(message, bb) {
  ...
  // optional LoginMessage login_message = 2;
  let $login_message = message.login_message;
  if ($login_message !== undefined) {
    writeVarint32(bb, 18);
    let nested = popByteBuffer();
    _encodeLoginMessage($login_message, nested);
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 2: {
        ...
        message.login_message = _decodeLoginMessage(bb);
        bb.limit = limit;
        break;
      }
